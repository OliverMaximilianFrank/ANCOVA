...
# Ich möchte aus theoretischen Überlegungen eine ANCOVA durchführen
...
# H1: Fahrer:innen mit niedriger Fahrpraxis (routine -> Gruppe 1) benötigen eine 
# signifikant längere Zeit zum Einleiten des Bremsvorganges (time) als Fahrer:innen 
# mit mittlerer oder hoher Fahrpraxis (routine -> Gruppe 2, Gruppe 3) bei Kontrolle 
# des Einflusses des Fahrer:innenalters (age).
...
plot(x=df$age,                            
     y=df$time, 
     main="Zsh. Alter und Reaktionszeit aufgeteilt nach Gruppenzugehörigkeit", 
     xlab = "Alter", 
     ylab = "Reaktionszeit") 
...
# Dies kann natürlich nur unter der Annahme behauptet werden, dass mein
...
# somit den tatsächlichen Einfluss auf unsere Regressionsgerade wiederspiegelt, würde 
# ich mich dazu entscheiden auch eben hauptsächlich diese beiden Datenpunkte 34 und 38 
...
# 4 weitere Punkte, die mein Interesse in geringem Maß geweckt haben sind einmal 
...
# In jedem Falle möchte ich die Daten so wie sie sind beibehalten und die Auffälligkeiten 
# nicht etwa aus dem Datensatz entfernen, was ich für schlechte Forscherpraxis halten würde.
